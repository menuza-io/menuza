import { and, db, eq, Organization, OrganizationLocation } from '@repo/database'
...
/**
 * Public endpoint: default active location id for a published org site.
...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	const [location] = await db
		...
		.innerJoin(
			Organization,
			eq(Organization.id, OrganizationLocation.organizationId),
		)
		.where(
			and(
