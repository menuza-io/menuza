...
const ORGANIZATION_LOCATION_TABLE_SQL = `CREATE TABLE IF NOT EXISTS \`OrganizationLocation\` (
...
export async function ensureOrganizationLocationTable() {
	await db.run(sql.raw(ORGANIZATION_LOCATION_TABLE_SQL))
	await db.run(
		sql.raw(
			'CREATE INDEX IF NOT EXISTS `OrganizationLocation_organizationId_idx` ON `OrganizationLocation` (`organizationId`)',
		),
	)
	await db.run(
		sql.raw(
...
export async function ensureOrganizationLocationColumns() {
	await ensureOrganizationLocationTable()
